...
export const Developer = (argument) => {
	...
	let url1 = `https://api.rawg.io/api/games?key=${API_KEY}&developers=${argument}`;
	...
	const RenderPublishers = async () => {
      ...
      <h1 class="title-white" style="margin-left:45px; margin-bottom: 30px"> Les titres du développeur <span class="text-underline"> ${transString(
				argument
			)} </span>  </h1>
      `;
		...
		console.log(document.getElementsByClassName("transition-trigger"));
		transitionLayer();
